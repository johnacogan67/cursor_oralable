...
Flash: [FIRMWARE_1.0.84_FLASH.md](../firmware/FIRMWARE_1.0.84_FLASH.md) · prior [FIRMWARE_1.0.82_FLASH.md](../firmware/FIRMWARE_1.0.82_FLASH.md)
...
```mermaid
flowchart LR
  Flash[Flash 1.0.84] --> Case[Case charge STAT]
...
| # | Step | Pass |
|---|------|------|
| A1 | Flash `merged.hex` **1.0.84** (SWD), verify `006` reads **1.0.84** | ☐ |
...
| A8 | TestFlight: gate accepts 1.0.63+; recommends 1.0.84; **Automatic** + Device LED mirror | ☐ |
...
### B1 — Charger LED matrix (STAT activity · 1.0.84)
...
| Expect (1.0.84) | LED | byte0 `on_dock` | byte4 `charge_active` | Notes |
|-----------------|-----|-----------------|------------------------|-------|
...
|---------|--------------|
| Wrong LED colour | Confirm FW **1.0.84** + placement; STAT flash≠taper; use app **Device LED** mirror |
