...
- Participants acknowledge the device is **not a medical device** at this stage.  
- Data is for **product validation and investor diligence**.  
- Optional: `PilotDataManager` and `Anonymizer` if IDs are published.  
- Ed and Pedro, as protocol leads, document consent for recruits.
...
*After sign-off, notify Balance Points and Ken with an updated data room index.*
