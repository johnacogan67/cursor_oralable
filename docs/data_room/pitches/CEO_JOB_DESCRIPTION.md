# Chief Executive Officer — Oralable / JAC Dental Solutions Limited

**Status:** Working draft for Nigel / Amstrow candidate referrals · **23 Jul 2026**  
**Entity:** JAC Dental Solutions Limited (Ireland) · CRO **697987** · product brand **Oralable**  
**Reports to:** Board (founder majority owner + Independent Director)  
**Works with:** Founder as **Executive Chairman** (product, clinical science, IP) — not a “Chief Scientist only” demotion of the founder  

This is a **search brief**, not an offer. Pay and equity bands are for discussion with counsel and the board.

**Send with:** company pitch for candidates — [PITCH_CEO_CANDIDATE.md](./PITCH_CEO_CANDIDATE.md) · [PITCH_CEO_CANDIDATE.pdf](./PITCH_CEO_CANDIDATE.pdf) · browser [PITCH_CEO_CANDIDATE.html](./PITCH_CEO_CANDIDATE.html)

---

## 1. Company in one paragraph

Oralable is a **temple-worn optical wearable** (PPG + motion) for overnight **vitals first** (HR / SpO₂), then jaw-load / bruxism-related metrics (IR-DC, TFI, SASHB) as a **Stage A wellness product**. A **Stage B medical-device** path comes later — not the current App Store or pilot claim. Hardware is Gen1 (Nordic nRF, magnetic charging case); iOS patient app in TestFlight (**4.3.3** build **5** · FW **1.0.84**); **Oralable Research Kits** for Ed/Pedro stay **gated / not yet shipped** (target **5 by 31 Aug 2026**) until the charge-to-temple gate clears (ready ≠ delivered — [VERSION_ALIGNMENT.md](./VERSION_ALIGNMENT.md) · [ORALABLE_RESEARCH_KIT.md](../clinical/ORALABLE_RESEARCH_KIT.md)). Near-term capital target is BalancePoints-style **Point B ~€180k** (F&F + EI PSSF CLN + HPSU), then a larger pre-seed/seed when evidence and governance allow.

---

## 2. Why this hire

The founder built the product, IP story, and technical stack and stays **Executive Chairman** on product / science / patents. The company needs a **commercial CEO** who can:

1. Close near-term funding and run a professional raise  
2. Run day-to-day ops (partnerships, team)  
3. Keep claims honest (wellness now; medical later)  
4. Work with the founder without taking over technical ownership  

---

## 3. Role summary

**Full-time (or near full-time) CEO** of an early-stage Irish hardware + software company, based **Ireland / UK** with travel as needed (investors, clinical partners, EMS/Kaga, US later).

**Mission (next 12–18 months):** Move Oralable from Point A (working Gen1 stack, thin cash, pilot pending) through **Point B funding**, solid **patient pilot evidence**, a credible **soft commercial launch** of the Stage A wearable, and a board-ready plan for the next equity round — without overselling medical claims.

---

## 4. Key responsibilities

### Capital & investors
- Own the **funding roadmap**: F&F pack, EI PSSF / HPSU path, then pre-seed/seed story  
- Build investor materials with the founder (truthful Stage A story)  
- Lead investor talks; close rounds on board-approved terms  
- Guard runway (cash now depends on founder loans)

### Commercial & GTM
- Define and run **Stage A GTM** (consumer / dental-adjacent wellness; Ireland/UK first)  
- Pricing, channels, early revenue — without professional clinical claims before the product is ready  
- Partnerships (clinical observers, KOLs, distribution) that match wellness claims  

### Operations & team
- Day-to-day company leadership; hiring plan within budget  
- Work with Cosec / finance (Amstrow) on books, registers, R&D reclaim hygiene  
- Vendor and supply relationships (EMS, Kaga, app contractors) at the commercial layer  

### Governance & risk
- Report to the board; support Independent Director process  
- Keep **regulatory posture honest**: wellness Stage A; no premature FDA/CE marketing  
- Cap table and option scheme hygiene with counsel  

### Founder partnership
- Weekly operating cadence with Executive Chairman  
- Founder owns: product roadmap, firmware/app technical direction, IP with counsel, clinical protocol quality  
- CEO owns: capital, commercial, ops, external narrative for investors/customers  

---

## 5. Success in the first 6–12 months (outcomes)

| Horizon | Outcomes that matter |
|---------|----------------------|
| **0–90 days** | Point B path live (F&F materials + EI process); Conor/cap-table cleanup supported; charge-to-temple gate cleared → **5 Research Kits** shipped / first field evidence in motion |
| **3–6 months** | Point B (or bridge) closed or clearly on track; Stage A soft-launch plan funded; team/contractor plan clear |
| **6–12 months** | Credible usage / pilot metrics; next-round story ready; optional medical exploration only as a **gated** workstream |

---

## 6. Profile we want

**Must-have**
- Raised capital for early-stage companies (ideally Ireland/UK/EU: angels, EI, pre-seed/seed)  
- Comfortable as **operating CEO** of a small hardware + software company (not advisory-only)  
- Clear speaker; can sell a **technical product without overclaiming**  
- **Regulated-adjacent** or consumer health / medtech / deep tech (wellness today, medical path later)  
- Based in or willing to work primarily in **Ireland or UK**; available for Dublin cadence with founder and Amstrow  

**Strong plus**
- Enterprise Ireland / HPSU / PSSF familiarity  
- Wearables, dental, sleep, or digital health GTM  
- Board-level experience with founder-led companies  
- Network for F&F and early institutional intros  

**Not a fit**
- Wants founder pushed into a pure “scientist” box with no board/product authority  
- Leads with medical-device claims before Stage A evidence  
- Equity expectations that assume **~28% day-one issued** founder transfer without milestones (board expects a **market early-stage CEO package**, typically **options with vesting / milestones** — see §7)

---

## 7. Package (indicative — for Nigel / candidates; board + counsel final)

| Element | Working band (discussion) |
|---------|---------------------------|
| **Cash** | Modest / deferred until Point B; then board-set lean Irish early-stage salary |
| **Equity** | **~5–10% fully diluted options** (illustrative centre **~6–8% FD**) with time vesting + milestones — see §7a |
| **Title** | Chief Executive Officer |
| **Founder** | Remains majority owner path (subject to Conor SPA) and **Executive Chairman** |

Exact numbers are **not** fixed in this JD — they are set by board resolution and legal docs.  
**Not in scope:** ~**28% day-one issued** founder share transfer without vesting.

### 7a. Illustrative early-stage CEO option package (discussion frame — not an offer)

**Instrument:** share options (or equivalent under an approved scheme), not a large block of issued ordinary shares on appointment.

**Size (discussion):** **~6–8% FD** inside a wider **5–10% FD** band for a full-time incoming CEO at pre-revenue / Point B stage.

**Time vesting (typical):**
- **4 years** total  
- **1-year cliff** (e.g. ~25% at month 12)  
- then monthly or quarterly thereafter  
- unvested options **lapse** if the CEO leaves (good/bad leaver rules in the scheme)

**Milestone vesting (illustrative split of the grant):**

| Slice | ≈ of grant | Example trigger |
|------:|-----------:|-----------------|
| Appointment | 10–15% | Signed CEO contract + start date |
| Near-term capital | 20–25% | Friends & Family **€50k** closed (or first Point B tranche) |
| Point B | 25–30% | Point B stack closed (**€180k** path: F&F + PSSF + HPSU as board-agreed) |
| Tenure / ops | remainder | Continues on the **4-year time vest** (and/or still CEO at 12–24 months) |

**Design intent:** equity is **earned** through fundraising and tenure — not a title gift. Board + counsel set final % , pool size, strike, and leaver terms.

---

## 8. How candidates will work with the current team

| Person | Role |
|--------|------|
| **John Cogan** | Founder · majority owner · **Executive Chairman** — product, IP, clinical/science |
| **Nigel Woods (Amstrow)** | Independent Director · Cosec / finance support |
| **CEO (this role)** | Capital, commercial, ops, company leadership |
| Contractors / partners | Firmware, iOS, manufacturing as today |

---

## 9. Process (for Nigel)

1. Nigel shares this brief with trusted candidates (confidential).  
2. Shortlist intro to John (+ Nigel as Independent Director).  
3. Product / Stage A walkthrough (no medical theatre).  
4. Board discusses package; counsel drafts term sheet / options.  
5. No public “CEO appointed” language until contract signed.

**Confidentiality:** Cap table cleanup and any parallel discussions with other advisors are sensitive — share JD and high-level company description only unless John authorises deeper diligence.

---

## 10. Email to Nigel (complete — copy/paste)

**To:** Nigel Woods  
**Subject:** Oralable — CEO role brief for candidate intros

Nigel,

Thanks again for offering to think about CEO candidates from your network.

I’ve written a short job description of what I need (attached / linked: *CEO_JOB_DESCRIPTION*). Summary:

- I need a **commercial CEO** to own fundraising, go-to-market, and day-to-day company leadership.
- I stay **Executive Chairman** on product, clinical/science, and IP — not a “Chief Scientist only” role with no board/product authority.
- Product truth for candidates: **Stage A** is a **temple** wellness wearable (HR/SpO₂ first, then jaw-load metrics). Medical-device claims are a **later** gate, not the current pilot or App Store story.
- Near-term priorities: close the **Point B** path (~**€180k**: F&F + EI PSSF + HPSU), get the Ed/Pedro pilot into the field once the charge gate clears, and keep the company investable with honest claims.

On package, for anyone you introduce, frame equity as a **normal early-stage option package**, not a large day-one share transfer:

- Discussion band roughly **5–10% fully diluted options** (centre of gravity ~**6–8%**), subject to board and counsel.
- **4-year vesting** with a **1-year cliff**.
- Additional vesting / unlocks tied to milestones (e.g. appointment, F&F, Point B close, tenure).
- I am **not** looking to transfer about a third of my holding as issued shares on appointment.

If you have people who fit — especially those who have raised early capital in Ireland/UK and can operate a small hardware + software company — I’d be grateful for confidential intros. Happy to do a short call with you first if you want to sanity-check the brief or the package band.

Thanks,  
John

---

*Related:* [CURRENT_GOVERNANCE_STATUS.md](../governance/CURRENT_GOVERNANCE_STATUS.md) · [KEN_PRESEED_STRUCTURE_EVAL.md](../governance/KEN_PRESEED_STRUCTURE_EVAL.md) · [FUNDING_POINT_B_AND_CAP_TABLE.md](../governance/FUNDING_POINT_B_AND_CAP_TABLE.md) · [COST_AND_TIMELINE.md](../governance/COST_AND_TIMELINE.md) · [../IP_NORTH_STAR.md](../IP_NORTH_STAR.md)
