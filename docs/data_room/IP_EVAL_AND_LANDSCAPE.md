...
3. **Product–claim mismatch remains the central strategic issue** — Stage A ships **temple / Temporalis**; foundation protects **intraoral masseter** optical monitoring. Stage A commercial IP rides on provisional **64/033,978**.
...
6. **Remaining Ken gaps:** sign AoA · UP receipt · clarify inv. “Spain” · provisional→JAC · Lisa Curley deed · Track 4 · temple FTO · post-RCE US status.
...
```mermaid
flowchart TB
  subgraph foundation [Foundation_WO2022234145]
    IE[IE_priority_2021_2022]
    PCT[PCT_May_2022]
    US[US_18289827_RCE_Jun2026]
    EP[EP_4333691_granted_Jul2026]
  end
  subgraph stageA [StageA_product_IP]
    PROV[US_provisional_64033978]
    TEMP[Temporalis_OMG_IRDC_TFI_SASHB]
  end
  subgraph aux [Auxiliary]
    CASE[Track4_case_enclosure]
  end
  IE --> PCT --> US
  PCT --> EP
  PROV --> TEMP
  foundation -.->|"OMG_reserved"| PROV
```
...
Counsel owns exact claim language. Engineering implements enabling embodiments ([../IP_NORTH_STAR.md](../IP_NORTH_STAR.md) §4).
...
**Counsel strategy already chosen (keep):** Track 2 minimalist response; do not stuff Temporalis / Airway Rescue into the old claim set during RCE.
...
1. **Foundation (now EP-granted):** tooth-fixed buccal support + optical sensor facing masseter + interior muscle hemodynamics / OMG — not bite-force pads, not surface-only reflection, not skin-topography facial sensors.
...
**Ingestion rule:** unpublished claim text stays in Drive / counsel vault; git gets status fields, dates, and high-level themes only.
...
*Last updated: 2026-07-22 (counsel pack ingested) · Related: [IP_PORTFOLIO_STATUS.md](./IP_PORTFOLIO_STATUS.md) · [../IP_NORTH_STAR.md](../IP_NORTH_STAR.md) · [PITCH_DECK_KEN.md](./PITCH_DECK_KEN.md)*
